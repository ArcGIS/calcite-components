import { h as k, L as y, k as l, n as w, x as a, s as o, j as z } from "./iframe.js";
import { n as T } from "./ref.js";
import { i as m } from "./keyed.js";
import { f as A, s as $, u as E } from "./dom.js";
import { g as p } from "./component.js";
import { c as F, s as S, a as O } from "./loadable.js";
import { N as B } from "./locale.js";
import { o as f } from "./openCloseComponent.js";
import { K as q } from "./resources3.js";
import { u as I } from "./useT9n.js";
import { D as c, C as s, S as r } from "./resources4.js";
/*!
...
class M extends y {
  ...
  async load() {
    S(this), this.open && f(this);
  }
  willUpdate(e) {
    e.has("active") && (this.hasUpdated || this.active !== !1) && this.handleActiveChange(), e.has("open") && (this.hasUpdated || this.open !== !1) && this.openHandler(), e.has("autoCloseDuration") && (this.hasUpdated || this.autoCloseDuration !== "medium") && this.updateDuration(), e.has("queue") && (this.hasUpdated || this.queue !== "last") && this.handleQueueChange(), e.has("numberingSystem") && this.numberingSystemChange(), e.has("messages") && this.effectiveLocaleChange();
  }
  loaded() {
    O(this);
  }
  ...
  handleActiveChange() {
    this.clearAutoCloseTimeout(), this.active && this.autoClose && !this.autoCloseTimeoutId && (this.initialOpenTime = Date.now(), this.autoCloseTimeoutId = window.setTimeout(() => this.closeAlert(), c[this.autoCloseDuration]));
  }
  openHandler() {
    f(this), this.open ? n.registerElement(this.el) : n.unregisterElement(this.el);
  }
  updateDuration() {
  ...
  // #region Rendering
  render() {
    const { open: e, autoClose: t, label: i, placement: b, active: d, openAlertCount: v } = this, C = t ? "alert" : "alertdialog", u = !e, h = E(q, this.icon, this.kind), x = v > 1;
    return this.el.inert = u, this.el.ariaLabel = i, this.el.toggleAttribute("calcite-hydrated-hidden", u), this.el.role = C, a`<div class=${o({
      [s.container]: !0,
      [s.containerActive]: d,
      [`${s.container}--${b}`]: !0,
      [s.containerEmbedded]: this.embedded,
      [s.focused]: this.isFocused
    })} @pointerenter=${this.autoClose && this.autoCloseTimeoutId ? this.handleMouseOver : null} @pointerleave=${this.autoClose ? this.handleMouseLeave : null} ${T(this.setTransitionEl)}>${h && this.renderIcon(h) || ""}<div class=${o(s.textContainer)} @focusin=${this.autoClose && this.autoCloseTimeoutId ? this.handleKeyBoardFocus : null} @focusout=${this.autoClose ? this.handleKeyBoardBlur : null}><slot name=${r.title}></slot><slot name=${r.message}></slot><slot name=${r.link}></slot></div>${this.renderActionsEnd()}${x ? this.renderQueueCount() : null}${this.renderCloseButton()}${e && d && t ? a`<div class=${o(s.dismissProgress)}></div>` : null}</div>`;
  }
  renderCloseButton() {
