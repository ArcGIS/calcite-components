import { h as E, L as H, k as m, x as d, s as l, E as c, j as A } from "./iframe.js";
...
import { c as K, s as N, a as q } from "./loadable.js";
...
class J extends H {
  ...
  async load() {
    (this.autoplay === "" || this.autoplay) && this.autoplay !== "paused" ? this.handlePlay(!1) : this.autoplay === "paused" && (this.paused = !0), N(this);
  }
  willUpdate(e) {
  ...
  handleArrowClick(e) {
    ...
    t === "next" ? (this.direction = "forward", this.nextItem(!0)) : t === "previous" && (this.direction = "backward", this.previousItem());
  }
  ...
  renderRotationControl() {
    const e = this.playing ? this.messages.pause : this.messages.play;
    return d`<button .ariaLabel=${e} class=${l({
      [a.paginationItem]: !0,
      [a.autoplayControl]: !0
    })} @click=${this.toggleRotation} title=${e ?? c}><calcite-icon .icon=${this.playing ? p.pause : p.play} scale=s></calcite-icon>${this.playing && d`<calcite-progress class=${l(a.autoplayProgress)} .label=${this.messages.carouselItemProgress} .value=${this.slideDurationRemaining}></calcite-progress>` || ""}</button>`;
  }
  ...
  renderPaginationItems() {
    ...
    return d`<div .ariaLabel=${r} class=${l(a.paginationItems)} role=tablist>${F(i, (o) => o.id, (o, n) => {
      const h = i.length, u = n === e, S = n === 0, T = n === h - 1, I = h - t - 1, f = e < t, k = e >= I, x = f ? 0 : e - Math.floor(t / 2), P = k ? h : x + t, w = f ? 0 : k ? I : x, D = f ? t + 1 : P, O = !S && !T && !u && (n === w - 1 || n === D), $ = u || n <= D && n >= w - 1, C = h - 1 <= t, z = u ? p.active : p.inactive;
