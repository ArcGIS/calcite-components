import { h as R, L as A, k as f, x as v, s as u, j as H } from "./iframe.js";
import { n as I } from "./ref.js";
import { i as F } from "./keyed.js";
import { i as M, b as y, s as o, h as S, c as C, n as P } from "./date.js";
/*!
...
class z extends A {
  ...
  getPreviousMonthDays(e, a, t) {
    const s = new Date(a, e, 0), n = s.getDate(), i = s.getDay(), r = [];
    if (i === (t + k) % m)
      return r;
    if (i === t)
      return [n];
    for (let c = (m + i - t) % m; c >= 0; c--)
      r.push(n - c);
    return r;
  }
  /**
  ...
  isHoverInRange() {
    if (!this.hoverRange || !this.startDate)
      return !1;
    const { start: e, end: a } = this.hoverRange, t = this.isFocusedOnStart(), s = this.startDate && a > this.startDate, n = this.endDate && a < this.endDate, i = this.startDate && e > this.startDate, r = this.endDate && e < this.endDate, c = !t && this.startDate && s && (!this.endDate || n), h = t && this.startDate && i && r;
    return c || h;
  }
  isRangeHover(e) {
    if (!this.hoverRange)
      return !1;
    const { start: a, end: t } = this.hoverRange, s = this.isFocusedOnStart(), n = this.isHoverInRange(), i = e > a && e < this.startDate, r = e < t && e > this.endDate, c = e > t && e < this.endDate, h = e < a && e > this.startDate, g = e < t && e > this.startDate, d = e > a && e < this.endDate, l = this.startDate && this.endDate;
    if (n) {
      if (l)
        return s ? e < this.endDate && (h || i) : c || r;
  ...
  getDays(e, a, t, s = "start") {
    ...
    return [
      ...e.map((d) => ({
        ...
        day: d,
        dayInWeek: c(),
        date: new Date(h, n - 1, d)
      })),
      ...a.map((d) => {
        const l = new Date(h, n, d), p = o(l, /* @__PURE__ */ new Date());
        return {
  ...
  // #region Rendering
  render() {
    const e = this.activeDate.getMonth(), a = this.activeDate.getFullYear(), t = this.localeData.weekStart % 7, { abbreviated: s, short: n, narrow: i } = this.localeData.days, r = this.scale === "s" ? i || n || s : n || s || i, c = [...r.slice(t, 7), ...r.slice(0, t)], h = this.getCurrentMonthDays(e, a), g = this.getPreviousMonthDays(e, a, t), d = this.getNextMonthDays(e, a, t), l = e + 1, p = this.getPreviousMonthDays(l, a, t), w = this.getCurrentMonthDays(l, a), b = this.getNextMonthDays(l, a, t), $ = this.getDays(g, h, d), x = this.getDays(p, w, b, "end");
   ...
   */
  renderDateDay({ active: e, currentMonth: a, currentDay: t, date: s, day: n, dayInWeek: i, ref: r }, c) {
    ...
    })} .currentMonth=${a} .dateTimeFormat=${this.dateTimeFormat} .day=${n} .disabled=${!h} .endOfRange=${this.isEndOfRange(s)} .highlighted=${this.betweenSelectedRange(s)} @calciteInternalDayHover=${this.dayHover} @calciteInternalDaySelect=${this.daySelect} .range=${!!this.startDate && !!this.endDate && !o(this.startDate, this.endDate)} .rangeEdge=${i === 0 ? "start" : i === 6 ? "end" : void 0} .rangeHover=${h && this.isRangeHover(s)} .scale=${this.scale} .selected=${this.isSelected(s)} .startOfRange=${this.isStartOfRange(s)} .value=${s} ${I((g) => {
  ...
  renderMonthCalendar(e, a, t = !1) {
    return v`<div class=${u({ [D.month]: !0 })} @keydown=${this.keyDownHandler}><div class=${u({ [D.weekHeaderContainer]: !0 })} role=row>${e.map((n) => v`<span class=${u({ [D.weekHeader]: !0 })} role=columnheader>${n}</span>`)}</div><div class=${u({ [D.weekDays]: !0 })} role=row>${a.map((n, i) => this.renderDateDay(n, t ? 50 + i : i))}</div></div>`;
  }
}
