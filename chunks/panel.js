import { h as w, L as F, k as b, x as n, s as i, E as x, j as k } from "./iframe.js";
...
import { c as T, s as I, a as M } from "./loadable.js";
...
class R extends F {
  ...
  async load() {
    I(this), this.isClosed = this.closed;
  }
  willUpdate(e) {
  ...
  // #region Rendering
  renderHeaderContent() {
    const { heading: e, headingLevel: a, description: o, hasHeaderContent: l } = this, c = e ? O({ class: t.heading, level: a, children: e }) : null, h = o ? n`<span class=${i(t.description)}>${o}</span>` : null;
  ...
  renderHeaderActionsEnd() {
    ...
    c === "up" && v.reverse();
    const $ = h ? n`<calcite-action .ariaExpanded=${!l} .ariaLabel=${p} .icon=${l ? v[0] : v[1]} id=${A.collapse} @click=${this.collapse} .scale=${this.scale} .text=${p} title=${(l ? f : p) ?? x}></calcite-action>` : null, C = o ? n`<calcite-action .ariaLabel=${m} .icon=${u.close} id=${A.close} @click=${this.handleUserClose} .scale=${this.scale} .text=${m} title=${m ?? x}></calcite-action>` : null, H = n`<slot name=${s.headerActionsEnd} @slotchange=${this.handleHeaderActionsEndSlotChange}></slot>`, z = e || $ || C || g;
    return d("header-actions-end", n`<div class=${i({ [t.headerActionsEnd]: !0, [t.headerActions]: !0 })} .hidden=${!z}>${H}${this.renderMenu()}${$}${C}</div>`);
  }
  renderMenu() {
    const { hasMenuItems: e, messages: a, menuOpen: o, menuFlipPlacements: l, menuPlacement: c } = this;
    return d("menu", n`<calcite-action-menu .flipPlacements=${l ?? ["top", "bottom"]} .hidden=${!e} .label=${a.options} .open=${o} .overlayPositioning=${this.overlayPositioning} .placement=${c}><calcite-action .icon=${u.menu} .scale=${this.scale} slot=${L.trigger} .text=${a.options}></calcite-action><slot name=${s.headerMenuActions} @slotchange=${this.handleHeaderMenuActionsSlotChange}></slot></calcite-action-menu>`);
  }
  renderHeaderNode() {
