import { h as d, L as h, k as p, n as m, x as o, s as c, j as u } from "./iframe.js";
...
import { c as z, s as S, a as k } from "./loadable.js";
...
class F extends h {
  ...
  loaded() {
    k(this), typeof this.value == "string" && this.updateItemsFromValue(this.value), this.populateInternalSelect();
  ...
  // #region Rendering
  renderChevron() {
    return o`<div class=${c(n.iconContainer)}><calcite-icon class=${c(n.icon)} icon=chevron-down .scale=${T(this.scale)}></calcite-icon></div>`;
  }
  render() {
    const { disabled: e } = this;
    return C({ disabled: e, children: o`<div class=${c(n.wrapper)}><select aria-errormessage=${r.validationMessage} .ariaInvalid=${this.status === "invalid"} .ariaLabel=${N(this)} class=${c(n.select)} .disabled=${e} @change=${this.handleInternalSelectChange} ${v(this.storeSelectRef)}><slot></slot></select>${this.renderChevron()}${w({ component: this })}</div>${this.validationMessage && this.status === "invalid" ? L({ icon: this.validationIcon, id: r.validationMessage, message: this.validationMessage, scale: this.scale, status: this.status }) : null}` });
  }
}
u("calcite-select", F);
export {
