import { h as N, L as K, k as S, x as v, s as b, C as g, o as B, j as D } from "./iframe.js";
...
class R extends K {
  ...
  getKeyAdjustedSize(e) {
    ...
    if (a === "horizontal" ? o === "end" ? t === p[1] || t === r[0] : t === p[0] || t === r[1] : t === p[1] || (o === "end" ? t === r[0] : t === r[1])) {
      const y = e.shiftKey ? c : n;
      return a === "horizontal" ? s + f * y : l + f * y;
    }
  ...
  // #region Rendering
  renderHeader() {
    return z("header", v`<div class=${b(u.contentHeader)} .hidden=${!this.hasHeader}><slot name=${$.header} @slotchange=${this.handleHeaderSlotChange}></slot></div>`);
  }
  render() {
    const { collapsed: e, position: t, initialContentWidth: i, initialContentHeight: n, contentWidth: m, contentWidthMax: a, contentWidthMin: w, contentHeight: d, contentHeightMax: l, contentHeightMin: s, resizable: x, layout: h, displayMode: o } = this, c = M(this.el), C = o !== "float-content" && o !== "float" && x, k = C ? h === "horizontal" ? d ? { height: `${d}px` } : null : m ? { width: `${m}px` } : null : null, r = !e && C ? z("separator", v`<div .ariaLabel=${this.messages.resize} .ariaOrientation=${h === "horizontal" ? "vertical" : "horizontal"} .ariaValueMax=${h == "horizontal" ? l : a} .ariaValueMin=${h == "horizontal" ? s : w} .ariaValueNow=${h == "horizontal" ? d ?? n : m ?? i} class=${b(u.separator)} @keydown=${this.separatorKeyDown} role=separator tabindex=0 touch-action=none ${E(this.connectSeparator)}></div>`) : null, p = () => h === "horizontal" ? t === "start" ? g.calciteAnimateInDown : g.calciteAnimateInUp : c === "ltr" && t === "end" || c === "rtl" && t === "start" ? g.calciteAnimateInLeft : g.calciteAnimateInRight, f = z("content", v`<div class=${b({
      [g.rtl]: c === "rtl",
      [u.content]: !0,
    ...
    })} .hidden=${e} style=${B(k)} ${E(this.storeContentEl)}>${this.renderHeader()}<div class=${b(u.contentBody)}><slot></slot></div>${r}</div>`), H = [z("action-bar", v`<slot name=${$.actionBar} @slotchange=${this.handleActionBarSlotChange}></slot>`), f];
    return t === "end" && H.reverse(), v`<div class=${b({ [u.container]: !0, [u.floatAll]: o === "float-all" })}>${H}</div>`;
  }
}
