import { x as l, s as r, d as g, h as f, L as S, k as p, E as m, j as I } from "./iframe.js";
import { n as v } from "./ref.js";
import { b as o, d as y } from "./dom.js";
...
class P extends S {
  ...
  registerItem(e) {
    const t = e.target, { content: i, position: n } = e.detail;
    this.itemMap.set(t, { position: n, content: i }), this.enabledItems = this.filterItems(), e.stopPropagation();
  }
  updateItem(e) {
  ...
  determineActiveStepper() {
    ...
    this.multipleViewMode = i !== "horizontal-single", e.forEach((n, s) => {
      n.hidden = i === "horizontal-single" && s !== (t || 0);
    });
  }
  getEnabledStepIndex(e, t = "next") {
    const { items: i, currentActivePosition: n } = this;
    let s = e;
    for (; i[s]?.disabled && this.layout !== "horizontal-single"; )
      s = s + (t === "previous" ? -1 : 1);
  ...
  handleDefaultSlotChange(e) {
    const t = y(e).filter((n) => n?.tagName === "CALCITE-STEPPER-ITEM");
    this.items = t;
  ...
  // #region Rendering
  render() {
    return this.el.ariaLabel = this.messages.label, this.el.role = "region", l`<div class=${r({ container: !0, [c.singleView]: this.layout === "horizontal-single" })} ${v(this.setContainerEl)}>${this.layout === "horizontal-single" && l`<div class=${r({ [c.stepBarContainer]: !0 })}>${this.items.map((e, t) => E({ active: t === this.currentActivePosition, complete: e.complete && t !== this.currentActivePosition && !e.error, disabled: e.disabled && t !== this.currentActivePosition, error: e.error && t !== this.currentActivePosition }))}</div>` || ""}${this.layout === "horizontal-single" && l`<div class=${r({ [c.actionContainer]: !0 })}>${this.renderAction("start")}${this.renderAction("end")}</div>` || ""}<slot @slotchange=${this.handleDefaultSlotChange}></slot></div>`;
  }
  renderAction(e) {
    const t = e === "start", i = t ? "chevron-left" : "chevron-right", { currentActivePosition: n, multipleViewMode: s, layout: d } = this, u = this.items.length, b = `${this.guid}-${t ? "start" : "end"}`;
    return d === "horizontal-single" && !s ? l`<calcite-action alignment=center appearance=transparent class=${r({
      [c.actionIcon]: !0
    })} compact data-position=${e ?? m} .disabled=${n === 0 && t || n === u - 1 && !t} .icon=${i} icon-flip-rtl id=${b ?? m} @click=${this.handleActionClick} .scale=${this.scale} .text=${t ? this.messages.previousStep : this.messages.nextStep}></calcite-action>` : null;
  }
}
I("calcite-stepper", P);
export {
