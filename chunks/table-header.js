import { h as d, L as h, x as i, s as o, E as n, j as p } from "./iframe.js";
import { e as u, n as m } from "./ref.js";
import { c as b, s as g, a as w } from "./loadable.js";
...
class R extends h {
  ...
  // #region Lifecycle
  async load() {
    g(this), this.updateScreenReaderText();
  }
  willUpdate(t) {
  ...
  render() {
    ...
    })} colSpan=${this.colSpan ?? n} @blur=${this.onContainerBlur} @focus=${this.onContainerFocus} .role=${this.parentRowType === "head" ? "columnheader" : "rowheader"} rowSpan=${this.rowSpan ?? n} scope=${t ?? n} .tabIndex=${this.selectionCell ? 0 : a ? -1 : 0} ${m(this.containerEl)}>${this.heading && i`<div class=${o(e.heading)}>${this.heading}</div>` || ""}${this.description && i`<div class=${o(e.description)}>${this.description}</div>` || ""}${this.selectionCell && this.selectionMode === "multiple" && i`<calcite-icon class=${o({ [e.active]: s || l })} .icon=${r} .scale=${C(this.scale)}></calcite-icon>` || ""}${(this.selectionCell || this.numberCell) && i`<span .ariaLive=${this.focused ? "polite" : "off"} class=${o(e.assistiveText)}>${this.screenReaderText}</span>` || ""}</th>`;
  }
}
p("calcite-table-header", R);
export {
  R as TableHeader
};
