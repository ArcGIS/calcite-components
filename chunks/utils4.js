import { g as Z } from "./_commonjsHelpers.js";
/*!
...
}, V = Q, tt = Array.prototype.concat, nt = Array.prototype.slice, _ = P.exports = function(n) {
  for (var e = [], r = 0, s = n.length; r < s; r++) {
    var a = n[r];
    V(a) ? e = tt.call(e, nt.call(a)) : e.push(a);
  }
  return e;
};
_.wrap = function(t) {
...
var et = P.exports, M = q, F = et, T = Object.hasOwnProperty, D = /* @__PURE__ */ Object.create(null);
for (var O in M)
  T.call(M, O) && (D[M[O]] = O);
var y = N.exports = {
  to: {},
...
y.get.rgb = function(t) {
  if (!t)
    return null;
  var n = /^#([a-f0-9]{3,4})$/i, e = /^#([a-f0-9]{6})([a-f0-9]{2})?$/i, r = /^rgba?\(\s*([+-]?\d+)(?=[\s,])\s*(?:,\s*)?([+-]?\d+)(?=[\s,])\s*(?:,\s*)?([+-]?\d+)\s*(?:[,|\/]\s*([+-]?[\d\.]+)(%?)\s*)?\)$/, s = /^rgba?\(\s*([+-]?[\d\.]+)\%\s*,?\s*([+-]?[\d\.]+)\%\s*,?\s*([+-]?[\d\.]+)\%\s*(?:[,|\/]\s*([+-]?[\d\.]+)(%?)\s*)?\)$/, a = /^(\w+)$/, o = [0, 0, 0, 1], i, l, h;
  if (i = t.match(e)) {
    for (h = i[2], i = i[1], l = 0; l < 3; l++) {
      var b = l * 2;
      o[l] = parseInt(i.slice(b, b + 2), 16);
    }
    h && (o[3] = parseInt(h, 16) / 255);
  } else if (i = t.match(n)) {
    for (i = i[1], h = i[3], l = 0; l < 3; l++)
      o[l] = parseInt(i[l] + i[l], 16);
    h && (o[3] = parseInt(h + h, 16) / 255);
  } else if (i = t.match(r)) {
    for (l = 0; l < 3; l++)
      o[l] = parseInt(i[l + 1], 0);
    i[4] && (i[5] ? o[3] = parseFloat(i[4]) * 0.01 : o[3] = parseFloat(i[4]));
  } else if (i = t.match(s)) {
    for (l = 0; l < 3; l++)
      o[l] = Math.round(parseFloat(i[l + 1]) * 2.55);
    i[4] && (i[5] ? o[3] = parseFloat(i[4]) * 0.01 : o[3] = parseFloat(i[4]));
  } else return (i = t.match(a)) ? i[1] === "transparent" ? [0, 0, 0, 0] : T.call(M, i[1]) ? (o = M[i[1]], o[3] = 1, o) : null : null;
  for (l = 0; l < 3; l++)
    o[l] = w(o[l], 0, 255);
...
y.get.hsl = function(t) {
  ...
  if (e) {
    var r = parseFloat(e[4]), s = (parseFloat(e[1]) % 360 + 360) % 360, a = w(parseFloat(e[2]), 0, 100), o = w(parseFloat(e[3]), 0, 100), i = w(isNaN(r) ? 1 : r, 0, 1);
    return [s, a, o, i];
  }
  return null;
};
...
c.rgb.hsl = function(t) {
  const n = t[0] / 255, e = t[1] / 255, r = t[2] / 255, s = Math.min(n, e, r), a = Math.max(n, e, r), o = a - s;
  let i, l;
  a === s ? i = 0 : n === a ? i = (e - r) / o : e === a ? i = 2 + (r - n) / o : r === a && (i = 4 + (n - e) / o), i = Math.min(i * 60, 360), i < 0 && (i += 360);
  const h = (s + a) / 2;
  return a === s ? l = 0 : h <= 0.5 ? l = o / (a + s) : l = o / (2 - a - s), [i, l * 100, h * 100];
};
c.rgb.hsv = function(t) {
  let n, e, r, s, a;
  const o = t[0] / 255, i = t[1] / 255, l = t[2] / 255, h = Math.max(o, i, l), b = h - Math.min(o, i, l), m = function(Y) {
    return (h - Y) / 6 / b + 1 / 2;
  };
  return b === 0 ? (s = 0, a = 0) : (a = b / h, n = m(o), e = m(i), r = m(l), o === h ? s = r - e : i === h ? s = 1 / 3 + n - r : l === h && (s = 2 / 3 + e - n), s < 0 ? s += 1 : s > 1 && (s -= 1)), [
    s * 360,
    a * 100,
    h * 100
  ];
...
c.rgb.hwb = function(t) {
  const n = t[0], e = t[1];
  let r = t[2];
  const s = c.rgb.hsl(t)[0], a = 1 / 255 * Math.min(n, Math.min(e, r));
  return r = 1 - 1 / 255 * Math.max(n, Math.max(e, r)), [s, a * 100, r * 100];
};
c.rgb.cmyk = function(t) {
  const n = t[0] / 255, e = t[1] / 255, r = t[2] / 255, s = Math.min(1 - n, 1 - e, 1 - r), a = (1 - n - s) / (1 - s) || 0, o = (1 - e - s) / (1 - s) || 0, i = (1 - r - s) / (1 - s) || 0;
  return [a * 100, o * 100, i * 100, s * 100];
};
function st(t, n) {
...
c.rgb.xyz = function(t) {
  ...
  const s = n * 0.4124 + e * 0.3576 + r * 0.1805, a = n * 0.2126 + e * 0.7152 + r * 0.0722, o = n * 0.0193 + e * 0.1192 + r * 0.9505;
...
c.rgb.lab = function(t) {
  ...
  const a = 116 * r - 16, o = 500 * (e - r), i = 200 * (r - s);
  return [a, o, i];
};
c.hsl.rgb = function(t) {
  ...
  for (let h = 0; h < 3; h++)
    a = n + 1 / 3 * -(h - 1), a < 0 && a++, a > 1 && a--, 6 * a < 1 ? o = i + (s - i) * 6 * a : 2 * a < 1 ? o = s : 3 * a < 2 ? o = i + (s - i) * (2 / 3 - a) * 6 : o = i, l[h] = o * 255;
  return l;
};
c.hsl.hsv = function(t) {
  ...
  r *= 2, e *= r <= 1 ? r : 2 - r, s *= a <= 1 ? a : 2 - a;
  const o = (r + e) / 2, i = r === 0 ? 2 * s / (a + s) : 2 * e / (r + e);
  return [n, i * 100, o * 100];
};
c.hsv.rgb = function(t) {
  const n = t[0] / 60, e = t[1] / 100;
  let r = t[2] / 100;
  const s = Math.floor(n) % 6, a = n - Math.floor(n), o = 255 * r * (1 - e), i = 255 * r * (1 - e * a), l = 255 * r * (1 - e * (1 - a));
  switch (r *= 255, s) {
    case 0:
      return [r, l, o];
    case 1:
...
c.hsv.hsl = function(t) {
  ...
  o = (2 - e) * r;
  const i = (2 - e) * s;
  return a = e * s, a /= i <= 1 ? i : 2 - i, a = a || 0, o /= 2, [n, a * 100, o * 100];
};
c.hwb.rgb = function(t) {
  ...
  return [h * 255, b * 255, m * 255];
};
c.cmyk.rgb = function(t) {
  const n = t[0] / 100, e = t[1] / 100, r = t[2] / 100, s = t[3] / 100, a = 1 - Math.min(1, n * (1 - s) + s), o = 1 - Math.min(1, e * (1 - s) + s), i = 1 - Math.min(1, r * (1 - s) + s);
  return [a * 255, o * 255, i * 255];
};
c.xyz.rgb = function(t) {
  const n = t[0] / 100, e = t[1] / 100, r = t[2] / 100;
  let s, a, o;
  return s = n * 3.2406 + e * -1.5372 + r * -0.4986, a = n * -0.9689 + e * 1.8758 + r * 0.0415, o = n * 0.0557 + e * -0.204 + r * 1.057, s = s > 31308e-7 ? 1.055 * s ** (1 / 2.4) - 0.055 : s * 12.92, a = a > 31308e-7 ? 1.055 * a ** (1 / 2.4) - 0.055 : a * 12.92, o = o > 31308e-7 ? 1.055 * o ** (1 / 2.4) - 0.055 : o * 12.92, s = Math.min(Math.max(0, s), 1), a = Math.min(Math.max(0, a), 1), o = Math.min(Math.max(0, o), 1), [s * 255, a * 255, o * 255];
};
c.xyz.lab = function(t) {
  ...
  const s = 116 * e - 16, a = 500 * (n - e), o = 200 * (e - r);
  return [s, a, o];
};
c.lab.xyz = function(t) {
  const n = t[0], e = t[1], r = t[2];
  let s, a, o;
  a = (n + 16) / 116, s = e / 500 + a, o = a - r / 200;
  const i = a ** 3, l = s ** 3, h = o ** 3;
  return a = i > 8856e-6 ? i : (a - 16 / 116) / 7.787, s = l > 8856e-6 ? l : (s - 16 / 116) / 7.787, o = h > 8856e-6 ? h : (o - 16 / 116) / 7.787, s *= 95.047, a *= 100, o *= 108.883, [s, a, o];
};
...
c.lch.lab = function(t) {
  const n = t[0], e = t[1], s = t[2] / 360 * 2 * Math.PI, a = e * Math.cos(s), o = e * Math.sin(s);
  return [n, a, o];
};
c.rgb.ansi16 = function(t, n = null) {
  const [e, r, s] = t;
  let a = n === null ? c.rgb.hsv(t)[2] : n;
  if (a = Math.round(a / 50), a === 0)
    return 30;
  let o = 30 + (Math.round(s / 255) << 2 | Math.round(r / 255) << 1 | Math.round(e / 255));
  return a === 2 && (o += 60), o;
};
...
c.ansi16.rgb = function(t) {
  ...
  const e = (~~(t > 50) + 1) * 0.5, r = (n & 1) * e * 255, s = (n >> 1 & 1) * e * 255, a = (n >> 2 & 1) * e * 255;
  return [r, s, a];
};
...
c.hex.rgb = function(t) {
  ...
  const r = parseInt(e, 16), s = r >> 16 & 255, a = r >> 8 & 255, o = r & 255;
  return [s, a, o];
};
c.rgb.hcg = function(t) {
  const n = t[0] / 255, e = t[1] / 255, r = t[2] / 255, s = Math.max(Math.max(n, e), r), a = Math.min(Math.min(n, e), r), o = s - a;
  let i, l;
  return o < 1 ? i = a / (1 - o) : i = 0, o <= 0 ? l = 0 : s === n ? l = (e - r) / o % 6 : s === e ? l = 2 + (r - n) / o : l = 4 + (n - e) / o, l /= 6, l %= 1, [l * 360, o * 100, i * 100];
};
...
c.hcg.rgb = function(t) {
  ...
  const s = [0, 0, 0], a = n % 1 * 6, o = a % 1, i = 1 - o;
  let l = 0;
  switch (Math.floor(a)) {
    case 0:
      s[0] = 1, s[1] = o, s[2] = 0;
...
c.hwb.hcg = function(t) {
  ...
  let a = 0;
  return s < 1 && (a = (r - s) / (1 - s)), [t[0], s * 100, a * 100];
};
...
function lt(t, n) {
  ...
  for (; n[s].parent; )
    e.unshift(n[s].parent), r = it(z[n[s].parent][s], r), s = n[s].parent;
  return r.conversion = e, r;
}
var ct = function(t) {
  const n = at(t), e = {}, r = Object.keys(n);
  for (let s = r.length, a = 0; a < s; a++) {
    const o = r[a];
    n[o].parent !== null && (e[o] = lt(o, n));
  }
  return e;
};
const E = W, ht = ct, k = {}, ut = Object.keys(E);
function ft(t) {
  const n = function(...e) {
    const r = e[0];
...
ut.forEach((t) => {
  k[t] = {}, Object.defineProperty(k[t], "channels", { value: E[t].channels }), Object.defineProperty(k[t], "labels", { value: E[t].labels });
  const n = ht(t);
  Object.keys(n).forEach((r) => {
    const s = n[r];
    k[t][r] = dt(s), k[t][r].raw = ft(s);
  });
});
var gt = k;
const x = rt, v = gt, X = [
...
for (const t of Object.keys(v))
  L[[...v[t].labels].sort().join("")] = t;
const $ = {};
function p(t, n) {
  if (!(this instanceof p))
    return new p(t, n);
  if (n && n in X && (n = null), n && !(n in v))
  ...
  if (t == null)
    ...
    this.color = I(s, r), this.valpha = typeof t[r] == "number" ? t[r] : 1;
...
p.prototype = {
  ...
  round(t) {
    return t = Math.max(t || 0, 0), new p([...this.color.map(bt(t)), this.valpha], this.model);
  },
  alpha(t) {
  ...
  hex(t) {
    return t !== void 0 ? new p(t) : x.to.hex(this.rgb().round().color);
  },
  hexa(t) {
    if (t !== void 0)
      return new p(t);
    ...
    return e.length === 1 && (e = "0" + e), x.to.hex(n) + e;
  },
  rgbNumber() {
    const t = this.rgb().color;
  ...
  mix(t, n) {
    ...
    const e = t.rgb(), r = this.rgb(), s = n === void 0 ? 0.5 : n, a = 2 * s - 1, o = e.alpha() - r.alpha(), i = ((a * o === -1 ? a : (a + o) / (1 + a * o)) + 1) / 2, l = 1 - i;
    return p.rgb(
      i * e.red() + l * r.red(),
      i * e.green() + l * r.green(),
      i * e.blue() + l * r.blue(),
...
for (const t of Object.keys(v)) {
  ...
  p.prototype[t] = function(...e) {
    return this.model === t ? new p(this) : e.length > 0 ? new p(e, t) : new p([...vt(v[this.model][t].raw(this.color)), this.valpha], t);
  }, p[t] = function(...e) {
    let r = e[0];
...
}, zt = mt("#007AC2"), $t = "calcite-color-", Ot = {
  r: 255,
  g: 255,
  b: 255
}, wt = {
...
}, It = 1, Rt = /^[0-9A-F]$/i, kt = /^#[0-9A-F]{3}$/i, xt = /^#[0-9A-F]{6}$/i, Mt = /^#[0-9A-F]{4}$/i, At = /^#[0-9A-F]{8}$/i, _t = (t) => Number((t * 100).toFixed()), Gt = (t) => Number((t / 100).toFixed(2));
function K(t, n = !1) {
  return R(t, n) || Ft(t, n);
}
function j(t, n, e) {
...
function R(t, n = !1) {
  return j(t, n ? 5 : 4, n ? Mt : kt);
}
function Ft(t, n = !1) {
  return j(t, n ? 9 : 7, n ? At : xt);
}
function Bt(t, n = !1, e = !1) {
...
function G(t) {
  const { r: n, g: e, b: r } = t, s = H(n), a = H(e), o = H(r), i = "a" in t ? H(t.a * 255) : "";
  return `#${s}${a}${o}${i}`.toLowerCase();
}
function H(t) {
...
function B(t, n = !1) {
  ...
  if (t.length === 3 || t.length === 4) {
    ...
    e = parseInt(`${i}${i}`, 16), r = parseInt(`${l}${l}`, 16), s = parseInt(`${h}${h}`, 16), a = parseInt(`${b}${b}`, 16) / 255;
  } else
    e = parseInt(t.slice(0, 2), 16), r = parseInt(t.slice(2, 4), 16), s = parseInt(t.slice(4, 6), 16), a = parseInt(t.slice(6, 8), 16) / 255;
  return isNaN(a) ? { r: e, g: r, b: s } : { r: e, g: r, b: s, a };
}
const J = (t) => t, f = J({
  HEX: "hex",
  HEXA: "hexa",
...
function Kt(t, n) {
  ...
  if (n)
    return e;
  const s = St.l.preview.size - r.size;
  return e + s;
}
export {
  f as C,
